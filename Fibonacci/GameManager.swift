...
class GameManager: NSObject {
     ...
     */
    func startNewSession(with scene: M2Scene?) {
        if grid {
            grid.removeAllTiles(animated: false)
        }
        
        if !grid || grid.dimension != GSTATE.dimension {
            grid = M2Grid(dimension: GSTATE.dimension)
            grid.scene = scene
        }
     ...
     */
    func move(to direction: M2Direction) {
        var tile: M2Tile? = nil
        
        // Remember that the coordinate system of SpriteKit is the reverse of that of UIKit.
        let reverse: Bool = direction == M2DirectionUp || direction == M2DirectionRight
        ...
        if direction == M2DirectionUp || direction == M2DirectionDown {
            grid.forEach({ position in
                if (tile = grid.tile(at: position)) != nil {
                    ...
                    while iterate(i, reverse, grid.dimension, -1) {
                        let t: M2Tile? = grid.tile(atPosition: M2PositionMake(i, position.y))
                        ...
                        if t == nil {
                            ...
                            if GSTATE.gameType == M2GameTypePowerOf3 {
                                let further: M2Position = M2PositionMake(i + unit, position.y)
                                let ft: M2Tile? = grid.tile(at: further)
                                if ft != nil {
                    ...
                    if target != position.x {
                        tile?.move(toCell: grid.cell(atPosition: M2PositionMake(target, position.y)))
                        pendingScore += 1
                    }
                }
            ...
            grid.forEach({ position in
                if (tile = grid.tile(at: position)) != nil {
                    ...
                    while iterate(i, reverse, grid.dimension, -1) {
                        let t: M2Tile? = grid.tile(atPosition: M2PositionMake(position.x, i))
                        ...
                        if t == nil {
                            ...
                            var level: Int = 0
                            if GSTATE.gameType == M2GameTypePowerOf3 {
                                let further: M2Position = M2PositionMake(position.x, i + unit)
                                let ft: M2Tile? = grid.tile(at: further)
                                if ft != nil {
                                    level = tile?.merge3(to: t, andTile: ft) ?? 0
                                }
                    ...
                    if target != position.y {
                        tile?.move(toCell: grid.cell(atPosition: M2PositionMake(position.x, target)))
                        pendingScore += 1
                    }
                }
            }, reverseOrder: reverse)
        ...
        grid.forEach({ position in
            let tile: M2Tile? = grid.tile(at: position)
            if tile != nil {
    ...
    func adjacentMatchesAvailable() -> Bool {
        for i in 0..<grid.dimension {
            for j in 0..<grid.dimension {
                ...
                let tile: M2Tile? = grid.tile(atPosition: M2PositionMake(i, j))
                ...
                if GSTATE.gameType == M2GameTypePowerOf3 {
                    if (tile?.canMerge(withTile: grid.tile(atPosition: M2PositionMake(i + 1, j))) != nil && tile?.canMerge(withTile: grid.tile(atPosition: M2PositionMake(i + 2, j))) != nil) || (tile?.canMerge(withTile: grid.tile(atPosition: M2PositionMake(i, j + 1))) != nil && tile?.canMerge(withTile: grid.tile(atPosition: M2PositionMake(i, j + 2))) != nil) {
                        return true
                    }
                } else {
                    if tile?.canMerge(withTile: grid.tile(atPosition: M2PositionMake(i + 1, j))) != nil || tile?.canMerge(withTile: grid.tile(atPosition: M2PositionMake(i, j + 1))) != nil {
                        return true
                    }
                }
            }
